...
def landing_page():
    # Adjusted CSS for button uniformity and vertical position
    st.markdown(
        """
        <style>
        /* CSS to make navigation buttons have a uniform size/appearance */
        .stButton>button {
            width: 100%; /* Make button fill its column */
            height: 3.5rem; /* Set a consistent height */
            padding: 0.5rem 1rem;
        ...
        .center-content-moved-up {
            ...
            /* Further reduced padding-top to move content up slightly more */
            padding-top: 8vh; /* Adjusted from 10vh to 8vh */
            text-align: center;
        }
        .welcome-text {
            font-size: 2.2rem;
...
def main():
    ...
    # Adjust column ratios to help fit text better (e.g., reduce space for 'Start' and 'Analysis/Results')
    cols = st.columns([1, 1.2, 1.2, 1.3]) # Proportional width for 4 columns
    ...
    for i, (step, data) in enumerate(nav_status.items()):
        ...
        with cols[i]:
            ...
            if step <= current: # Allow navigating back to completed/current steps
                # Apply the CSS class defined in landing_page's style block to all nav buttons
    ...
    st.markdown("---") # Keep the horizontal line below the navigation
